...
while True:
    ...
    try:
        response = requests.get(
            f'{server_test_url}/{current_timestamp.strftime("%Y-%m-%d-%H-%M")}/json')
        data = response.json()
        for item in data:
            ...
            producteur.send('type_traffic', message_str.encode(
                'utf-8'), key.encode())

        time.sleep(1)
    except requests.exceptions.JSONDecodeError:
        print(
            "Erreur de décodage JSON : La réponse ne contient pas de données JSON valides.")
